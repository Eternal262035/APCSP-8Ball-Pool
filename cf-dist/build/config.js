export const mspt = 20; // ms per tick
export const msprt = 14; // ms per render tick
// these are the dimensions of a professional pool table (9 ft), according to google lol
/** the width of the pool table */
export const mapWidth = 1118 * 0.3;
/** the height of the pool table */
export const mapHeight = 2235 * 0.3;
// export const consoleLogs: boolean = true;
/** the size of a pool ball */
export const ballSize = 57.15 * 0.3 * 0.82; //*0.96;   // = 20.0025
export const ballSizeUncorrected = 57.15 * 0.3 * 1; //*0.96;   // = 20.0025
// shoudl probably make this smaller. 
