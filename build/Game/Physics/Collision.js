    ...
    for (let i = 0; i < entityManager.entities.size; i++) {
        for (let j = i + 1; j < entityManager.entities.size; j++) {
            ...
            if (dr < firstEntity.hitboxData.size + secondEntity.hitboxData.size) {
                ...
                const angle = Math.atan2(firstEntity.positionData.y - secondEntity.positionData.y, firstEntity.positionData.x - secondEntity.positionData.x);
                const scale = Math.cos(angle);
                const correctionVector = new Vector(firstEntity.positionData.x - secondEntity.positionData.x, firstEntity.positionData.y - secondEntity.positionData.y).scale(0.02);
