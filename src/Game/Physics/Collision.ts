    ...
    for (let i=0; i<entityManager.entities.size; i++) {
        for (let j=i+1; j<entityManager.entities.size; j++) {
            ...
            if (dr < firstEntity.hitboxData.size + secondEntity.hitboxData.size) {
                ...
                const mom1:VectorAbstract = firstEntity.physicsData.velocity.scale(firstEntity.physicsData.mass);
                const mom2:VectorAbstract = secondEntity.physicsData.velocity.scale(secondEntity.physicsData.mass);
